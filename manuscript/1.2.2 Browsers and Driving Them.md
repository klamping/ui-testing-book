...
Recently, Chrome has released support for their own protocol called "Chrome DevTools". WebdriverIO has added support for this protocol through [Puppeteer support](https://webdriver.io/docs/api/browser/getPuppeteer). The industry has evolved its tooling over the years and WebdriverIO has kept up giving you the flexibility to pick what works best.
...
When running your test, WebdriverIO will first check if a browser driver is running and available. If not, it will install Chromedriver, including the necessary browser, and get it running. Seeing that Chrome is the most popular browser in use as of the writing of this book, chances are you already have it installed.

In previous versions of this book, I had a whole section on how to configure all of this, but thankfully, WebdriverIO has continued to improve support so much that I was able to take all of that out. There's really not much you need to do, and you can honestly skip the rest of this chapter. 

### How To Use a 'Driver'?

If you are interested running your tests for other browsers besides Chrome, you'll want to use their browser driver. There are several WebDriver clients available, Selenium Server being the most popular. Let's walk through setting up one of these clients so that you can start writing tests.
...
### Installing and Running ChromeDriver
	...
	**Note:** WebDriverIO handles installing and running ChromeDriver automatically, so you don't need to go through this to run your tests. This content is simply in case you're curious to try things out.
