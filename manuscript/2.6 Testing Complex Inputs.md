...
If you're unfamiliar with using the Chrome DevTools to inspect the generated HTML of a website, have a read through [their documentation](https://developers.google.com/web/tools/chrome-devtools/dom) to become familiar with the very useful functionality it provides.
...
You can use characters like "Left arrow" or "Backspace", or in our instance "Enter", and WebdriverIO takes care of translating them into the unicode characters needed to represent those keys. The W3C has [a list of all supported keywords](https://w3c.github.io/webdriver/#keyboard-actions) available for your reference. 
...
Next, we use a `for ... of` loop to iterate through the tags passed in. If you're unfamiliar with this method, check out [the MDN documentation on it](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/for...in). Note that we don't use a [`forEach`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/forEach) loop, as the asynchronous nature of WebdriverIO makes using such functions a little more complicated than what it's worth.
...
Combined with the rest of our page object, here's how the entire thing looks:
...
While that's helpful, it's really only testing a simple numeric input. The numbers may change, but the data format doesn't. What we need is a full-featured utility that allows us to request random types of data. That's where a tool like [ChanceJS](https://chancejs.com/) can help out.
...
Instead, we can use [the built-in array `map` function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map) to pull information out of our array. `map` is very similar to `forEach`, if you've used that function before. The main difference is that `map` allows you to take the data in the array and return something else from it. 
...
```js
const tags = await article.$$tags.map($tag => {
    return $tag.getText();
});
```
...
Note that in this instance, we don't use an `await` for our assertion. The reason we can do this is that `tags` is not a WebdriverIO command, but rather the returned strings from our `map` call.
...
Note that the `tags` property will be `async` (even though we don't use that in our function definiton). So to get the value of this, we need to use `await` in our call:

```
const tags = await article.tags;
expect(tags).toEqual(articleDetails.tags);
```
Again, we don't have to use `await` in our assertion, because `tags` is already "awaited".

Alternative, we can skip storing our tags as a constant, and instead just reference the page object directly:

```
expect(await article.tags).toEqual(articleDetails.tags);
```

Here's how the entire Editor test looks with all our changes:
...
One weird thing with this code is that the `await` statement in the last assertion is in a different spot. This can be a little confusing for people who aren't familiar with the nuances of `await/async`, so you might want to put a comment in there explaing that this is indeed correct.

Alternatively, you could just include an unneeded `await` to (hopefully) avoid confusion:
...
describe('Post Editor', function () {
    ...
    describe('"Unsaved Changes" alerts', async function () {
    });
});
```
...
One very important part here is the addition of [`resolves`](https://jestjs.io/docs/expect#resolves) to our chain. This is necessary to tell ExpectJS that `browser.acceptAlert` is an asynchronous function and that it should wait for the promise to resolve before moving forward. 
...
There's one thing to know about our assertion: By calling the `browser.acceptAlert()` command in it, the alert dialog is accepted and our `browser.refresh()` command is fully executed.

If we were to have called `browser.dismissAlert()` instead, the `refresh` would not have occurred (it would have been cancelled), leaving our page in an "unsaved changes" state.

This could cause trouble if a follow-on test tried reloading the page (which it does as part of the `before` hook in our parent `describe` block). If you try to navigate via WebdriverIO when an alert is showing, you'll get this error: `unexpected alert open: {Alert text : }`. So just keep that in mind if it comes up during your testing. 
...
Why didn't we get the alert text in the test we just wrote? Well, to oversimplify, it was a different type of alert, and running `getAlertText` on it would result in a useless empty string. 

There's a quirk in how this website works in that one type of navigation triggers a `confirm` alert to show (which `getAlertText` can return a valid string from), and the other hooks into [BeforeUnloadEvent](https://developer.mozilla.org/en-US/docs/Web/API/BeforeUnloadEvent), which causes a dialog box to appear that cannot be read by `getAlertText`. 

For our new test, we'll trigger our link navigation through a simple click `$('=Home').click();`

Then we'll get the alert text: `const alertText = await browser.getAlertText();`

With that text, we can run our assertion: `expect(alertText).toEqual('Do you really want to leave? You have unsaved changes!');`

Again, we don't need to `await` the assertion because that's already done with our `const` assignment.
...
One final note: you can't use these `alert` commands to enter "basic auth" credentials. To do that, you need to pass in the credentials via your `browser.url` call:

`await browser.url('http://username:password@example.com/');`
